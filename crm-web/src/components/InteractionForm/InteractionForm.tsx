...
export function InteractionForm({ customerId, onCreate }: InteractionFormProps) {
    ...
    try {
        ...
        setJustSaved(true)
        successRef.current?.focus() // focus management — see item 3 below
    } catch (err) {
        const apiError = err instanceof ApiError ? err : new ApiError('Unknown error', 'network')
        setSubmitError(apiError)
        summaryRef.current?.focus() // send focus back to the field the error relates to
    } finally {
        setSubmitting(false)
    }
    }
    ...
    <form className="interaction-form" onSubmit={handleSubmit} aria-label="Add interaction">
        ...
        />
        <p id="summary-help">
        {summary.length} / {SUMMARY_MAX_LENGTH} characters. Do not enter payment details or passwords.
        </p>
        {submitError && (
